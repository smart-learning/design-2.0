...
@observer class AudioBookBuyPage extends React.Component {
		...
		return <View style={[ CommonStyles.container, { backgroundColor: '#ffffff' } ]}>
			<SafeAreaView style={{ flex: 1, width: '100%' }}>
				<ScrollView style={{ flex: 1 }}>
					...
					<View style={{ marginTop: 12 }}>
						<Text style={{ textAlign: 'center' }}>구매한 오디오북이 없습니다.</Text>
					</View>
					}
					{ this.store.list && this.store.list.map( ( item, key ) => {
						return (
							<SummaryListItem key={ key }
											 thumbnail={ ( item.data.images && item.data.images.list ) ? item.data.images.list : null }
