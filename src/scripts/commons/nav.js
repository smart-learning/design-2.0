...
export default {
  ...
  parseDeepLink: scheme => {
    try {
      ...
      switch (action) {
        ...
        case 'in_browser':
          store.inAppWebViewUrl = schemes[0];
          break;
  ...
  commonBack: () => {
    console.log('commonBack start:', store.prevLocations);
    if (store.prevLocations.length === 0) navigation.goBack();
    else {
      const screen = store.prevLocations.pop();
      navigation.navigate(screen);
      console.log('commonBack end:', store.prevLocations);

      //
      setTimeout(() => {
        const locs = store.prevLocations;
        const locsLen = locs.length;
        if (locsLen > 0) store.prevLocations.pop();
        console.log('정리:', store.prevLocations);
      }, 0);
    }
  }
};
