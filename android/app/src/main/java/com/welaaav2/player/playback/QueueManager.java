...
import com.welaaav2.player.utils.QueueHelper;
...
/**
 * Simple data provider for queues. Keeps track of a current queue and a current index in the queue.
 * Also provides methods to set the current queue based on common queries, relying on a given
 * MusicProvider to provide the actual media metadata.
 */
public class QueueManager {
  ...
  public QueueManager(@NonNull Resources resources, @NonNull MetadataUpdateListener listener) {
    ...
    mPlayingQueue = Collections.synchronizedList(new ArrayList<QueueItem>());
    mCurrentIndex = 0;
  }
  ...
  public boolean setQueueFromSearch(String query, Bundle extras) {
    List<MediaSessionCompat.QueueItem> queue =
        QueueHelper.getPlayingQueueFromSearch(query, extras);
    setCurrentQueue(mResources.getString(R.string.app_name), queue);
    updateMetadata();
    return queue != null && !queue.isEmpty();
  }

  public void setRandomQueue() {
    setCurrentQueue(mResources.getString(R.string.app_name),
