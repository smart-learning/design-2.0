package kr.co.influential.youngkangapp.player.utils;
...
public class MediaIDHelper {
   ...
   *
   * Encode the media browseable categories, if any, and the unique music ID, if any, into a single
   * String mediaID.
   *
   * MediaIDs are of the form <categoryType>/<categoryValue>|<musicUniqueId>, to make it easy to
   * find the category (like genre) that a music was selected from, so we can correctly build the
   * playing queue. This is specially useful when one music can appear in more than one list, like
   * "by genre -> genre_1" and "by artist -> artist_1".
   *
   ...
   * concatenation of category (eg "by_genre"), categoryValue (eg "Classical") and unique musicID.
  ...
  /**
   * Extracts category and categoryValue from the mediaID. mediaID is, by this sample's convention,
   * a concatenation of category (eg "by_genre"), categoryValue (eg "Classical") and mediaID. This
   * is necessary so we know where the user selected the music from, when the music exists in more
   * than one music list, and thus we are able to correctly build the playing queue.
   *
