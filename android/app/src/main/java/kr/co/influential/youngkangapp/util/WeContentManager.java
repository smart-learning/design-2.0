...
//        + "FROM DOWNLOAD WHERE NOT totalSize is NULL ";
  ...
  public void insertProgress(String cid, String duration, String PlayCount, String progress, String server_sync_flag
      ) throws Exception {
    if (!db.isOpen()) {
      openDb();
    }
    ...
    db.execSQL(String.format(
        "INSERT INTO PROGRESS ( cid, duration , playCount , progress , reg_date , server_sync_flag)"
            +
            "values ( '%s','%s', '%s', '%s', sysdate() , '%s');",
        cid, duration , PlayCount, progress , server_sync_flag));
  }
  ...
  public ArrayList<HashMap<String, Object>> getProgressCid(String cid) throws Exception {
    ...
    String query =
        "SELECT cid , duration , PlayCount , progress , reg_date , server_sync_flag FROM PROGRESS WHERE cid='"
            + cid + "'";
    Cursor c = db.rawQuery(query, null);
  ...
  public void updateProgress(String cid, String duration) throws Exception {
    ...
    String sql = "";
          sql =
          "UPDATE PROGRESS SET playCount= playCount+1 , reg_date='sysdate()' WHERE cid='" + cid + "'";
