...
typedef NS_ENUM (NSInteger, IFMiniPlayerViewMoveToPage)
{
    IFMiniPlayerViewMoveToPageNone,
    IFMiniPlayerViewMoveToPageClose
};

@interface IFDrmMiniPlayerUiView () < IFTouchViewDelegate, UIScrollViewDelegate >
{
    UIView *_playerView;
...
@implementation IFDrmMiniPlayerUiView
...
- (void) initSubviews
{
    ...
    _touchView = [[IFTouchView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_playBtn.frame) + 10, 0,
                                                               CGRectGetWidth(_playerView.frame) - (CGRectGetMaxX(_playBtn.frame) + 10),
                                                               _playerView.frame.size.height)];
...
- (void) resizeTimeLabel
{
    if ( _totalTimeLabel && _timeLabel )
    {
        ...
        _totalTimeLabel.frame = CGRectMake(_playerView.frame.size.width - 15.f - _totalTimeLabel.frame.size.width,
                                           ...
                                           _totalTimeLabel.frame.size.width, _totalTimeLabel.frame.size.height);
        ...
        _timeLabel.frame = CGRectMake(CGRectGetMinX(_totalTimeLabel.frame) - _timeLabel.frame.size.width - 3.f,
                                      ...
                                      _timeLabel.frame.size.width, _timeLabel.frame.size.height);
    }
    
    if ( _titleLabel )
    {
        _titleLabel.frame = CGRectMake(CGRectGetMaxX(_playBtn.frame) + 10.f, 0,
...
- (void) handlePanGesture: (UIPanGestureRecognizer *) gestureRecognizer
{
    ...
    else if ( gestureRecognizer.state == UIGestureRecognizerStateChanged )
    {
        ...
        [self moveFrameInitialized: IFMiniPlayerViewMoveToPageNone
                            offset: moveDistance
                          animated: NO];
    ...
    else if ( gestureRecognizer.state == UIGestureRecognizerStateEnded )
    {
        ...
        IFMiniPlayerViewMoveToPage status = IFMiniPlayerViewMoveToPageNone;
        CGFloat moveDistance = 0.f;
        
        if ( webOffset > (CGRectGetWidth(_playerView.frame) / 3) )
        {
            // history Back
            status = IFMiniPlayerViewMoveToPageClose;
...
- (void) moveFrameInitialized: (IFMiniPlayerViewMoveToPage) movePage
    ...
    if ( animated )
    {
        [UIView animateWithDuration: 0.3f
                         ...
                         completion: ^(BOOL finished)
                                    {
                                        if ( movePage == IFMiniPlayerViewMoveToPageClose )
                                        {
