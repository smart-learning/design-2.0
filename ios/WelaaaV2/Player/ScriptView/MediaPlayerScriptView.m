...
@interface MediaPlayerScriptView () < UITableViewDelegate, UITableViewDataSource, ScriptControlViewDelegate >
{
    UIView *_topLineView;
    ...
    PlayerScriptItemView *_scriptItemView;
    
    ScriptControlView *_controlView;
...
@implementation MediaPlayerScriptView
...
- (void) initSubviews
{
    _status = MediaPlayerScriptViewModeNone;
    ...
    _scriptItemView = [[PlayerScriptItemView alloc] initWithFrame : CGRectZero];
    ...
    _controlView = [[ScriptControlView alloc] initWithFrame: CGRectZero];
...
- (void) setStatus : (NSInteger) status
{
    ...
    else
    {
        ...
        if ( _scriptItemView )
        {
            _scriptItemView.hidden = (status != MediaPlayerScriptViewModeText);
        }
        
        if ( _controlView )
        {
            _controlView.hidden = (status != MediaPlayerScriptViewModeList);
        }
        
        if ( _tableView )
        {
            _tableView.hidden = (status != MediaPlayerScriptViewModeList);
        }
    }
...
- (void) setScript : (NSArray *) script
{
    if ( script.count > 0 )
    {
        NSMutableArray *tempArray = [[NSMutableArray alloc] initWithArray : script];
        [tempArray removeLastObject];
        _array = [[NSArray alloc] initWithArray : tempArray];
        
        //_array = [[NSArray alloc] initWithArray: script];     // 지식영상 자막 마지막에 </BODY></SAMI> 이 붙어서 삭제함.
        if ( _tableView )
        {
...
- (void) scriptWithTime : (NSTimeInterval) time
{
    ...
    if ( currentIndex > -1 && _array.count > currentIndex && currentIndex != _prevIndex )
    {
        ...
        NSIndexPath *prevIndexPath = [NSIndexPath indexPathForRow : _prevIndex
                                                        inSection : 0];
        PlayerScriptCell *prevCell = [_tableView cellForRowAtIndexPath : prevIndexPath];
        PlayerScriptCell *currentCell = [_tableView cellForRowAtIndexPath : currentIndexPath];
...
heightForRowAtIndexPath : (NSIndexPath *) indexPath
{
    ...
    return [PlayerScriptCell heightWithTime : [common convertTimeToString:startTime Minute:YES]
                                       text : memo
          ...
          cellForRowAtIndexPath : (NSIndexPath *) indexPath
{
    ...
    PlayerScriptCell *cell = [tableView dequeueReusableCellWithIdentifier : cellIdentifier];
    
    if ( !cell )
    {
        cell = [[PlayerScriptCell alloc] initWithStyle : UITableViewCellStyleDefault
...
- (void) pressedTabList
{
    if ( [self.delegate respondsToSelector : @selector(mediaPlayerScriptView:statusChange:)] )
    {
        [self.delegate mediaPlayerScriptView : self
                                statusChange : MediaPlayerScriptViewModeText];
    }
}
...
- (void) pressedTabText
{
    ...
    {
        [self.delegate mediaPlayerScriptView : self
                                statusChange : MediaPlayerScriptViewModeList];
    }
}

#pragma mark - ScriptControlViewDelegate
